...
// champion, awards) is editable by EVERYONE until this instant, then locks. Keep it
// before any group finishes (~Jun 24) so re-picking the R32 still carries real
// uncertainty (no advancement hindsight). Group-winner points are frozen to each
// player's kickoff snapshot regardless of edits. saveBracket/saveAwards write via the
// service role, so THIS constant — not the DB RLS — is the editing deadline.
export const FREE_EDIT_LOCK_MS = Date.parse("2026-06-23T07:30:00Z"); // Tue Jun 23, 00:30 PDT
